  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter analyses to show only current patient's analyses
        const allAnalyses = analysesResponse.analyses || [];
        const patientAnalyses = allAnalyses.filter(
          (analysis) =>
            analysis.userId === user?.id || analysis.patientId === user?.id
        );

        // Sort by date and get the 3 most recent
        const sortedAnalyses = patientAnalyses.sort(
                    ...
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span
                        className={`text-xs font-medium ${
                          analysis.aiResponse?.severity === "high"
                            ? "text-orange-600"
                            : analysis.aiResponse?.severity === "medium"
                            ? "text-yellow-600"
                            : "text-green-600"
                        }`}
                      >
